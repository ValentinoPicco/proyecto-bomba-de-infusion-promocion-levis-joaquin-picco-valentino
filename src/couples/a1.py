...
class ModeloAcopladoA1(CoupledDEVS):
    def __init__(self, nombre="Modelo_A1", escenario_ordenes=[]):
        """
        ...
        # Instanciamos los Submodelos (Componentes)
        self.generador = self.addSubModel(GeneradorDeOrdenesMedicas("Generador", escenario_ordenes))
        self.sensor = self.addSubModel(SensorDeFlujo("Sensor"))
        self.controlador = self.addSubModel(ControladorDeBomba("Controlador"))
